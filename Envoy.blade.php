...
@task('deploy', ['confirm' => true])
    ...
    export URL=`wget -qO- https://api.github.com/repos/CESNET/ciselnik/releases/latest | grep browser_download_url |
    cut -d'"' -f4`
...
@task('cleanup')
    cd {{ $dir }}
    find . -maxdepth 1 -name "ciselnik*" | sort | head -n -6 | xargs rm -rf
